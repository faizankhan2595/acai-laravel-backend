...
@section('content')
        ...
        <div class="row">
            <div class="col-xs-12">
                <div class="page-title-box">
                    <h4 class="page-title">Edit Sales Person</h4>
                ...
                <div class="card-box">
                    <div class="row">
                        ...
                        <div class="col-sm-12 col-xs-12 col-md-10 col-md-offset-1">
                            <div class="p-20">
                                <form action="{{ route('salesperson.update',$salesperson) }}" method="post" enctype="multipart/form-data" data-parsley-validate novalidate>
                                    @csrf
                                    @method('PUT')
                                    <div class="form-group">
                                        <label for="name">Full Name<span class="text-danger">*</span></label>
                                        <input type="text" name="name" placeholder="Enter name" class="form-control" id="name" parsley-trigger="change" required value="{{ $salesperson->name }}">
                                    </div>
                                    <div class="form-group">
                                        <label for="email">Email<span class="text-danger">*</span></label>
                                        <input type="text" name="email" placeholder="Enter email" class="form-control" id="email" parsley-trigger="change" required value="{{ $salesperson->email }}">
                                    </div>
                                    <div class="form-group">
                                        <label for="mobile_number">Mobile Number<span class="text-danger">*</span></label>
                                        <input type="text" name="mobile_number" placeholder="Enter Mobile Number" class="form-control" id="mobile_number" parsley-trigger="change" required value="{{ $salesperson->mobile_number }}">
                                    </div>

                                    <div class="form-group">
                                        <label for="dob">Birthday</label>
                                        <div class="input-group">
                                            <input type="text" class="form-control" name="dob" placeholder="mm/dd/yyyy" id="dob" value="{{ (!is_null($salesperson->dob)) ? $salesperson->dob->format('d-M-Y') : '' }}" readonly>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="gender" class="form-control" id="link">
                                            <option value="1" {{ ($salesperson->gender == 1) ? 'selected' : '' }}>Male</option>
                                            <option value="2" {{ ($salesperson->gender == 2) ? 'selected' : '' }}>Female</option>
                                        </select>
                                    </div>

                                    <div class="form-group">
                                        <label class="control-label">Avatar</label>
                                        @if ($salesperson->avatar != '')
                                            (<small class="text-primary"><a target="_blank" href="{{asset('storage/'.$salesperson->avatar)}}">View old Image</a></small>)
                                        @endif
                                        <input type="file" id="avatar" name="avatar" class="filestyle" data-buttonname="btn-primary">
                                    </div>
                                    <input type="hidden" name="old_avatar" value="{{ ($salesperson->avatar != '') ? $salesperson->avatar : '' }}">
                                    <input type="hidden" name="role" value="sales_person">
                                    <input type="hidden" name="sort_order" value="{{ $salesperson->sort_order }}">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="account_status" class="form-control" id="link">
                                            <option value="1" {{ ($salesperson->account_status == 1) ? 'selected' : '' }}>Active</option>
                                            <option value="0" {{ ($salesperson->account_status == 0) ? 'selected' : '' }}>Inactive</option>
                                        </select>
                                    </div>
                                    <div class="form-group text-right m-b-0">
                                        ...
                                        <a href="{{ route('salesperson.index') }}" type="reset" class="btn btn-default waves-effect m-l-5">
                                            Cancel
                                        </a>
                                    </div>
    ...
    <div id="change-password-modal" class="modal-demo">
        ...
        <form action="{{ route('salesperson.change',$salesperson) }}" method="post" data-parsley-validate novalidate id="change-password">
            @csrf
            @method('PUT')
...
@push('scripts')
    ...
    <script type="text/javascript">
        ...
        $("#change-password").on('submit',function(e) {
            e.preventDefault();
            $.ajax({
                url : '{{ route('salesperson.change',$salesperson) }}',
                type: 'POST',
