                ...
                <div class="card-box">
                    <div class="row">
                        ...
                        <div class="col-sm-12 col-xs-12 col-md-10 col-md-offset-1">
                            <div class="p-20">
                                <form action="{{ route('voucher.update',$voucher) }}" method="post" enctype="multipart/form-data" data-parsley-validate novalidate>
                                    @csrf
                                    @method('PUT')
                                    <div class="form-group">
                                        ...
                                        <select name="user_id" class="form-control" id="user_id" required>
                                            ...
                                            {{-- <option {{ (0 == $voucher->user_id) ? 'selected' : '' }} value="0">Project Acai</option> --}}
                                            @foreach ($merchants as $merchant)
                                                <option {{ ($merchant->id == $voucher->user_id) ? 'selected' : '' }} value="{{ $merchant->id }}">{{ $merchant->name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="voucher_type" class="form-control" id="voucher_type" required>
                                            <option value="1" {{ $voucher->voucher_type == 1 ? 'selected' : '' }}>Amount Discount</option>
                                            <option value="2" {{ $voucher->voucher_type == 2 ? 'selected' : '' }}>Percent Discount</option>
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        ...
                                        <select name="valid_for" class="form-control" id="valid_for" required>
                                            @foreach ($voucher->validFor as $key => $valid_for)
                                                <option value="{{ $key }}" {{ ($voucher->valid_for == $valid_for) ? 'selected' : '' }}>{{ $valid_for }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        <label for="name">Title<span class="text-danger">*</span></label>
                                        <input type="text" name="title" placeholder="Enter title" class="form-control" id="name" parsley-trigger="change" required value="{{ $voucher->title }}">
                                    </div>

                                    <div class="form-group">
                                        <label for="discount_title">Discount Title</label>
                                        <input type="text" name="discount_title" placeholder="Discount title" class="form-control" id="discount_title" value="{{ $voucher->discount_title }}">
                                    </div>
                                    <div class="form-group">
                                        <label for="discount_subtitle">Discount Sub-Title</label>
                                        <input type="text" name="discount_subtitle" placeholder="Discount Sub-title" class="form-control" id="discount_subtitle" value="{{ $voucher->discount_subtitle }}">
                                    </div>

                                    <div class="form-group">
                                        <label for="name">Price (Points required)<span class="text-danger">*</span></label>
                                        <input type="text" name="price" placeholder="Price" class="form-control" id="price" parsley-trigger="change" value="{{ $voucher->price }}">
                                    </div>

                                    <div class="form-group">
                                        <label for="expiring_on">Expiry Date<span class="text-danger">*</span></label>
                                        <div class="input-group">
                                            <input type="text" class="form-control" name="expiring_on" placeholder="Expiry date" id="expiring_on" value="{{ $voucher->expiring_on->format('d-M-Y') }}" readonly>
                                    ...
                                    <div class="form-group">
                                        ...
                                        @if ($voucher->image != '')
                                            (<small class="text-primary"><a target="_blank" href="{{asset('storage/'.$voucher->image)}}">View old Image</a></small>)
                                        @endif
                                        <input type="file" id="image" name="image" class="filestyle" data-buttonname="btn-primary">
                                    </div>
                                    <input type="hidden" name="old_image" value="{{ ($voucher->image != '') ? $voucher->image : '' }}">
                                    <div class="form-group">
                                        <label for="notes">Notes</label>
                                        <textarea class="form-control" name="notes" placeholder="Notes" id="notes">{{ $voucher->notes }}</textarea>
                                    </div>
                                    <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        <input type="checkbox" id="is_featured" name="is_featured" data-switch="primary" {{ ($voucher->is_featured == 1) ? 'checked' : '' }}/>
